...
- L15. Checkout: product subtotal + chosen logistics line. "I'll arrange my own logistics" option (Path B2). Total = product + logistics only, no added fee line.
  - **L15a (v1) — 🟡 done, needs re-verification S33/S34.** Per-vendor shipping display (firm/estimated), total calculation, `ownLogistics` suppresses shipping cost in `CartSummary`. Built S33; the `ownLogistics` piece needs a fresh checkout-page screenshot to confirm it actually renders correctly (see D-adjacent note in S33 handover) before calling it done. **Known gap, deferred on purpose:** actual order creation (`createOrderFromCart`) still doesn't pass `logisticsQuoteId`/`logisticsCost` — orders currently total product-only regardless of what checkout previews. Next step, not yet built.
  - **L15b — not started.** Harden and test the 2+-products-per-vendor path (`getShippingSummaryByVendor`'s multi-quote summing logic exists in code but has only been tested with a single product/single vendor so far).
  - **L15c — not started.** "Change shipping" / buyer-facing logistics-pool browser — lets a buyer who ticked "arrange own logistics" actually find a provider, rather than just opting out with nowhere to go. No buyer-facing pool browse page exists yet anywhere in the codebase (`sdk.logistics.searchProviders()` exists but is currently only called provider-side).
