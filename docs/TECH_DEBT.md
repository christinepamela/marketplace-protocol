...
TIER 3 — Frontend (🟡 In progress — L12–L15c done, L15d next)
...
**Priority:** Low standalone — folded into L16's testing. Note: L15c's full end-to-end test (provider sees buyer RFQ in opportunities, submits quote, buyer accepts in checkout UI, order created with logistics cost) is also deferred to L16 for the same reason — B15 blocks the opportunities page, so the provider side of the loop can't be confirmed live until L16 rebuilds it.
